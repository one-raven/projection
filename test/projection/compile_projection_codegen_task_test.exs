defmodule Projection.CompileProjectionCodegenTaskTest do
  ...
  test "projection.codegen emits typed list bindings from schema items option" do
    ...
    types_only =
      File.read!(Path.join([configured_ui_root(), "types", "#{screen_name}_types.slint"]))
  ...
  test "projection.codegen emits types/ directory, minimal types file for non-id_table screens, and build.rs include path" do
    cleanup =
    ...
    types_path = Path.join([configured_ui_root(), "types", "clock_types.slint"])
    ...
    assert build_rs =~ "with_include_paths"

    types_include =
      ["..", "..", configured_ui_root(), "types"]
      |> Path.join()
      |> String.replace("\\", "/")

    assert build_rs =~ types_include
    assert build_rs =~ "cargo:rerun-if-changed=#{types_include}/"
  end
